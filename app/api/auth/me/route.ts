...
export async function GET() {
  ...
  const result = await sql`
    SELECT id, email, username, bio, camera_brand, profile_picture_url, age, location, open_for_work, theme_color, crowns, nodes, created_at
    FROM users WHERE id = ${session.rows[0].user_id}
  `
