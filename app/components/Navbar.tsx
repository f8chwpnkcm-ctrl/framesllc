  ...
  return (
    <nav style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }} className="desktop-nav">
        <Link href="/feed" style={{ color: 'rgba(255,255,255,0.4)', fontSize: '13px', textDecoration: 'none' }}>Feed</Link>
      ...
      {menuOpen && (
        <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: '#0a0a0a', borderBottom: '0.5px solid rgba(255,255,255,0.08)', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '16px', zIndex: 99 }} className="mobile-menu">
          <Link href="/feed" onClick={() => setMenuOpen(false)} style={{ color: 'rgba(255,255,255,0.5)', fontSize: '15px', textDecoration: 'none' }}>Feed</Link>
          ...
          {user && <Link href={`/u/${user.username}`} onClick={() => setMenuOpen(false)} style={{ color: 'rgba(255,255,255,0.5)', fontSize: '15px', textDecoration: 'none' }}>My profile</Link>}
        </div>
      )}
