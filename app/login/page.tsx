    ...
    <main style={{ minHeight: '100vh', background: 'linear-gradient(to bottom, #0a0a0a, #111827)', fontFamily: 'var(--font-inter), sans-serif' }}>
      ...
      <div style={{ maxWidth: '680px', margin: '0 auto', padding: '60px 24px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '24px', marginBottom: '32px' }}>
          <div style={{ width: '88px', height: '88px', borderRadius: '50%', background: `rgba(${accent === '#FFE500' ? '255,229,0' : '255,255,255'},0.1)`, border: `2px solid ${accent}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden' }}>
        ...
        {/* Owner only prompt to upload avatar */}
        {isOwner && !user.profile_picture_url && (
          <div style={{ background: `${accent}10`, border: `0.5px solid ${accent}30`, borderRadius: '12px', padding: '20px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <div style={{ color: '#fff', fontSize: '14px', fontWeight: '700', marginBottom: '4px' }}>Add a profile picture</div>
              <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: '13px' }}>Let the community know who you are.</div>
            </div>
            <a href={`/u/${username}/edit`} style={{ background: accent, color: '#000', fontSize: '13px', fontWeight: '700', padding: '10px 18px', borderRadius: '8px', textDecoration: 'none' }}>Upload</a>
          </div>
        )}
      </div>
    </main>
